...
module.exports = function(RED) {
	let opened = null;				// LazuriteChannelNodeの競合確認
	...
	function LazuriteChannelNode(config) {
		...
		node.config = config;
		node.users = {};
		node.opened = false;				// 複数のLazuriteChannelNodeが存在した時に有効かどうかを確認する
		...
		node.register = function(n) {
			if(n) {
				...
				if((opened === null) && (node.opened === false)) {
					opened = node;
					node.opened = true;
					init();
				} else if((opened !== null) && (node.opened === false)) {
					RED.log.warn(`LazuriteChannelNode(${node.id}) is already used by ${opened.id}`);
				}
			}
		}
		...
		function init() {
			node.connecting = true, node.connected = false, node.closing = false;
